...
internal class Day04Part1 : CommonTask<Input, Int>(
    ...
    override fun calculateResult(input: Input): Int {
...
private val inputConverter: (String) -> Input = { s ->
    val sections = s.trim().split("\n\n")
    val numbers = sections.first().split(",").map{ it.toInt() }
    val boards = sections.drop(1).map { it.lines() }.map { it.map { it.split(" ").filter{ it.isNotBlank() }.map { it.toInt() } } }
